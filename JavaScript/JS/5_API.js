...
console.log(str.substring(2,4)); //2, 3번 인덱스 값 추출
...
Test4.oncilck = function(){
    ...
    // 즉, +을 제외한 산술연산시 문자열은 number자료형으로 자동형변환 된다

    console.log(7%'a'); //숫자형태가 아닌 문자열을 숫자로 형변환시 반환되는 값 (NaN).
...
test7.onclick = function(){
    /*
        자바스크립트에서 변수나 리터럴이 boolean형으로 
        형변환이 가능하다.
        - if(값), Boolean(값)

        값이 존재하지 않느 것들은 false로 변환된다
        ex) 0, -0, 0.0, "", underfined, null, NaN
...
test8.oncilck=function(){
   ...
   console.log(3==="3"); //t -> 자료형이 달라서 false
   console.log(3 === "3");

   console.log(3 !== "3"); //t
   console.log(3 === Numver("3")); //t
